...
async function getInstallations(searchQuery: string = "", page: number = 1) {
  try {
    ...
    const requestedInstallations = Array.isArray(allInstallations)
      ? allInstallations.filter(
          (inst: any) => inst.status?.toUpperCase() === "REQUESTED"
        )
      : [];

    return {
      installations: requestedInstallations,
      meta: {
        total: requestedInstallations.length,
        page: page,
        limit: 10,
        totalPages: Math.ceil(requestedInstallations.length / 10),
      },
      error: null,
    };
  } catch (error) {
...
export default async function RequestedInstallationsPage({
  searchParams,
}: {
