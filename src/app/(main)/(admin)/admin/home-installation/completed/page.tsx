...
async function getInstallations(searchQuery: string = "", page: number = 1) {
  try {
    ...
    const completedInstallations = Array.isArray(allInstallations)
      ? allInstallations.filter(
          (inst: any) => inst.status?.toUpperCase() === "COMPLETED"
        )
      : [];
    ...
    return {
      ...
      meta: {
        total: completedInstallations.length,
        page: page,
        limit: 10,
        totalPages: Math.ceil(completedInstallations.length / 10),
      },
      error: null,
    };
  } catch (error) {
